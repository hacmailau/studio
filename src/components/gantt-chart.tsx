...
export function GanttChart({
  ...
  useEffect(() => {
    ...
    svg
      .selectAll("rect.bar, g.bar-label")
      .transition()
      .duration(300)
      .style("opacity", (d: any) =>
        selectedHeatId === null || d.Heat_ID === selectedHeatId ? 1 : 0.45
      );
    ...
    svg
      ...
      .duration(300)
      .style("opacity", (d: any) =>
        selectedHeatId !== null && d.Heat_ID === selectedHeatId ? 0.9 : 0.3
      )
      .attr("stroke-width", (d: any) =>
        selectedHeatId !== null && d.Heat_ID === selectedHeatId ? 2 : 1.5
      );
  }, [selectedHeatId, heats]);

  useEffect(() => {
    if (!chartContainerRef.current || !tooltipRef.current || !yAxisRef.current)
      return;
    ...
    const drawD3Gantt = () => {
      ...
      const allOpsWithHeatInfo = heats
        .flatMap((heat) =>
      ...
      const { minTime, maxTime } = allOpsWithHeatInfo.reduce(
        (acc, op) => ({
          minTime: d3.min([acc.minTime, op.startTime])!,
          maxTime: d3.max([acc.maxTime, op.endTime])!,
        }),
        {
          minTime: allOpsWithHeatInfo[0].startTime,
          maxTime: allOpsWithHeatInfo[0].endTime,
        }
      );
      ...
      const totalTimeMinutes =
        (fullTimeDomainEnd.getTime() - fullTimeDomainStart.getTime()) / 60000;
      const visibleTimeMinutes = timeRange * 60;
      const width = Math.max(
        containerWidth,
        containerWidth * (totalTimeMinutes / visibleTimeMinutes)
      );

      const chartSvgElement = d3
        .select(chartOutputEl)
        .append("svg")
      ...
      const chartSvg = chartSvgElement
        .append("g")
        .attr("transform", `translate(${margin.left},${margin.top})`);

      const xScale = d3
        .scaleTime()
        .domain([fullTimeDomainStart, fullTimeDomainEnd])
        .range([0, width]);
      const yScale = d3
        .scaleBand()
        .domain(unitOrder)
      ...
      chartSvg
        ...
        .attr("width", (d) =>
          Math.max(1, xScale(d.endTime) - xScale(d.startTime))
        )
        ...
        .on("mousemove", (event, d) =>
          mousemove(
            event,
            `
            <div class="font-bold">Mẻ: ${d.Heat_ID} (#${
              d.sequenceInCaster
            })</div>
             <div>Mác: ${d.Steel_Grade}</div>
            <div>Thiết bị: ${d.unit}</div>
            <hr class="my-1"/>
            <div>Bắt đầu: ${d3.timeFormat("%H:%M %d/%m")(d.startTime)}</div>
            <div>Kết thúc: ${d3.timeFormat("%H:%M %d/%m")(d.endTime)}</div>
      ...
      const labels = chartSvg
        ...
        .attr(
          "transform",
          (d) =>
            `translate(${xScale(d.startTime) + 8}, ${
              (yScale(d.unit) ?? 0) + yScale.bandwidth() / 2
            })`
        )
        .style("pointer-events", "none");
